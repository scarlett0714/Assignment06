...
public class VolunteerManager {
	...
	public String addMember(String name, int gender, int age) {
		...
		Set<Map.Entry<String, Member>> set = member.entrySet();
		Iterator<Map.Entry<String, Member>> it3 = set.iterator();
		while(it3.hasNext()) {
			Map.Entry<String, Member> entry = it3.next();
	...
	public String addVolunteer(Volunteer v) {
		...
		Iterator<Volunteer> it2 = volunteer.iterator();
		while(it2.hasNext()) {
			Volunteer key = it2.next();
			if (v.volName.equals(key.volName)) {
				return "중복 등록 불가";
			}
		}
		volunteer.add(v);
